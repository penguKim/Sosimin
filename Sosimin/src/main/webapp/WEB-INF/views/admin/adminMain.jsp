...
// 								                    format: 'dd/MM/yy'
		            ...
		            <div class="col-12">
		              <div class="card recent-sales overflow-auto">
		                ...
		                <div class="card-body">
		                  <h5 class="card-title">최근 상품 <span>| 오늘</span></h5>
		                  ...
		                  <table class="table table-borderless datatable">
		                    ...
		                    <tbody>
		                      <c:forEach var="order" items="${recentProductList }">
		                      	<tr>
			                        <th scope="row" style="text-align: center;"><a href="SellerInfo?member_id=${order.product_buyer }">${order.order_id }</a></th>
			                        <td><a href="SellerInfo?member_id=${order.product_buyer }">${order.product_buyer }</a></td>
			                        <td ><a href="ProductDetail?product_id=${order.product_id}" ><span class="product_name">${order.product_name }</span></a></td>
			                        <td><fmt:formatNumber value="${order.product_price }" pattern="###,###"/>원</td>
			                        <td style="text-align: center;">
				                        <c:choose>
				                        	<c:when test="${order.order_status eq 1 }">
						                        <span class="badge bg-success">거래완료</span>
				                        	</c:when>
				                        	<c:when test="${order.order_status eq 0 }">
												<span class="badge bg-warning">거래중</span>
				                        	</c:when>
				                        </c:choose>
			                        </td>
		                      	</tr>
