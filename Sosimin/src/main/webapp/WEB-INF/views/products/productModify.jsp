					...
					<div id="divLine">
						...
						<ul id="ulLine">
							<li class="td">
								...
								<input type="text" id="myMap" name="trade_place"  size="64" required placeholder="지역을 설정해 주세요" value="부산광역시 ${guDong.gu} ${guDong.dong}">
							</li>
						</ul>
				</div>
			    ...
			    <ul id="ulLine">
			        <li class="trade_methodRadio">
			            ...
			            <label id="radioChoise2">
			                <input type="radio" id="trade_method2" name="trade_method" value="1" ${productModify.trade_method == 1 ? 'checked' : ''} <c:if test="${payStatus eq 0 }">disabled</c:if>> 소심거래<c:if test="${payStatus eq 0 }">(선택불가)</c:if>
			            </label><br>
			        </li>
			    </ul>
			</div>
