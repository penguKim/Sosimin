	...
	//			alert("확인");
		...
		$.ajax({
			...
			success: function(result) {
				if(result.length > 0) {
					for(let i = 0; i < ${fn:length(MyPageList)}; i++) {
						let order_id = $(".list" + i).data("order");
						let element = $('[data-id="' + $(".list" + i).data("id") + '"]');
						for(let id of result) {
							if(id.order_id == order_id) {
								// 후기 작성 버튼 비우기
								$(element).find("#singleProductButtonArea").empty();
								// 후기 보기 버튼 추가
								$(element).find("#singleProductButtonArea").append('<input type="button" class="reviewBtn_' + order_id + '" data-id="' + order_id + '" value="보낸 후기 보기" data-bs-toggle="modal" data-bs-target="#reviewModal_' + order_id + '" onclick="reviewView(this)">');
							}
						}
					}
				}
...
<!-- 										채팅 n회 -->
							...
							<c:when test="${mypage.trade_status eq '2' }"> <%-- 거래(판매) 완료 --%>
								<div class="single-block list${status.index} col-4" data-id="${mypage.product_id }" data-order="${mypage.order_id }" id="singleProductAreaDealComplete">
									<a href="ProductDetail?product_id=${mypage.product_id }">
										<img src="${pageContext.request.contextPath}/resources/upload/${mypage.product_image1}">
									</a>									
...
<!-- 									<span class="heart"></span> -->
	...
	<!-- 										채팅 n회 -->
									...
									<div id="singleProductButtonArea">
<%-- 										<c:choose> --%>
<%-- 											<c:when test="${mypage.reviewer_id ne sessionScope.sId }"> --%>
...
<%-- 											</c:when> --%>
<%-- 											<c:otherwise> --%>
<%-- 												<input type="button" class="reviewBtn_${mypage.order_id }" data-id="${mypage.order_id}" value="보낸 후기 보기" data-bs-toggle="modal" data-bs-target="#reviewModal_${mypage.order_id}" onclick="reviewView(this)"> --%>
<%-- 											</c:otherwise> --%>
<%-- 										</c:choose> --%>
									</div>
								</div>
								...
								<jsp:include page="review.jsp">
									<jsp:param name="orderId" value="${mypage.order_id}" />
								</jsp:include>	
							</c:when>
						</c:choose>
